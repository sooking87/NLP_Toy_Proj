...
lyrics_sentiment_dataset_3 = pd.read_csv('tcc_ceds_music.csv')
print(lyrics_sentiment_dataset_3.keys())
lyrics_sentiment_dataset_3 = lyrics_sentiment_dataset_3.drop(
    ['rank', 'last-week', 'peak-rank', 'weeks-on-board'], axis=1)
lyrics_sentiment_dataset_3.drop_duplicates(subset='song', inplace=True)
lyrics_sentiment_dataset_3.reset_index(drop=True)
...
sentiment1 = lyrics_sentiment_dataset_3.apply(
...
for i in lyrics_sentiment_dataset_3.index.tolist():
    print("\n%d번 째 인덱스" % i)
    lyrics_sentiment_dataset_3.loc[i, 'neg_sentiment'] = sentiment1[i]['neg']
    lyrics_sentiment_dataset_3.loc[i, 'neu_sentiment'] = sentiment1[i]['neu']
    lyrics_sentiment_dataset_3.loc[i, 'pos_sentiment'] = sentiment1[i]['pos']
    lyrics_sentiment_dataset_3.loc[i, 'com_sentiment'] = sentiment1[i]['compound']

lyrics_sentiment_dataset_3.to_csv("billboard_dataset.csv", index=False)
